...
describe("NFTMetadataLib Unit Tests", function () {
  async function deployFixture() {
    ...
    const NFTMetadataLibTester = await ethers.getContractFactory(
      "NFTMetadataLibTester"
    );
  ...
  describe("base64Encode", function () {
    ...
    it("Should encode ASCII text correctly", async function () {
      ...
      const decodedResult = Buffer.from(result, "base64")
        .toString("utf8")
        .replace(/\0+$/, "");
      expect(decodedResult).to.equal(testStr);
    });
    ...
    it("Should encode binary data correctly", async function () {
      ...
      // ผลลัพธ์ที่คาดหวังจากการเข้ารหัส Base64
      const decodedResult = Buffer.from(result, "base64");
      const originalData = Buffer.from(binaryData);
      expect(decodedResult.slice(0, originalData.length)).to.deep.equal(
        originalData
      );
    });
    ...
    it("Should handle data requiring padding", async function () {
      ...
      expect(
        Buffer.from(oneByteResult, "base64").toString().replace(/\0+$/, "")
      ).to.equal("A");
      ...
      expect(
        Buffer.from(twoByteResult, "base64").toString().replace(/\0+$/, "")
      ).to.equal("AB");
      ...
      expect(
        Buffer.from(threeByteResult, "base64").toString().replace(/\0+$/, "")
      ).to.equal("ABC");
    });
  });
  ...
  describe("Integration", function () {
    it("Should create valid NFT metadata JSON", async function () {
      ...
      const decodedStr = Buffer.from(base64Encoded, "base64")
        .toString("utf8")
        .replace(/\0+$/, "")
        .trim();
      try {
