...
describe("Membership Activity Simulation", function () {
  ...
  async function deploySimulationFixture() {
    ...
    const initialAmount = ethers.parseEther("1000"); // ให้เยอะหน่อยสำหรับ upgrade
    ...
    return { nft, usdt, owner, memberUsers };
  }

  // ฟังก์ชันสำหรับสุ่มการกระทำ
  class MembershipSimulator {
    constructor(nft, usdt, owner, users) {
      this.nft = nft;
      this.usdt = usdt;
      this.owner = owner;
      this.users = users;
    ...
    getRandomUnregisteredUser() {
      const unregistered = this.users.filter(user => !this.registeredMembers.has(user.address));
    ...
    async simulateRegistration() {
      ...
      try {
        console.log(`📝 ${user.address.slice(0, 8)}... สมัครสมาชิก Plan 1 (Upline: ${uplineAddress.slice(0, 8)}...)`);
        ...
        console.log(`✅ สมัครสมาชิกสำเร็จ! (สมาชิกทั้งหมด: ${this.registeredMembers.size})`);
        return true;
      } catch (error) {
    ...
    async simulateUpgrade() {
      ...
      try {
        console.log(`⬆️ ${user.address.slice(0, 8)}... อัพเกรด Plan ${currentPlan} → Plan ${nextPlan}`);
        ...
        const upgradeCost = ethers.parseEther(nextPlan.toString()) - ethers.parseEther(currentPlan.toString());
    ...
    async simulateExit() {
      ...
      try {
        ...
        console.log(`✅ ออกจากระบบสำเร็จ! (สมาชิกเหลือ: ${this.registeredMembers.size})`);
        return true;
      } catch (error) {
        console.log(`❌ ออกจากระบบล้มเหลว: ${error.message} (อาจยังไม่ครบ 30 วัน)`);
        return false;
      }
    }
    ...
    async performRandomAction() {
      ...
      const actions = [
        { name: 'register', weight: 50, action: () => this.simulateRegistration() },
    ...
    printSimulationStats() {
      ...
      console.log(`📝 การลงทะเบียน: ${this.simulationStats.registrations} ครั้ง`);
      console.log(`⬆️ การอัพเกรด: ${this.simulationStats.upgrades} ครั้ง`);
      console.log(`🚪 การออกจากระบบ: ${this.simulationStats.exits} ครั้ง`);
      console.log(`💰 รายได้รวม: ${ethers.formatEther(this.simulationStats.totalRevenue)} USDT`);
    ...
    async printSystemStats() {
      ...
      console.log(`   Total Commission: ${ethers.formatEther(stats[2])} USDT`);
      console.log(`   Owner Funds: ${ethers.formatEther(stats[3])} USDT`);
      console.log(`   Fee Funds: ${ethers.formatEther(stats[4])} USDT`);
      console.log(`   Fund Balance: ${ethers.formatEther(stats[5])} USDT`);
    }
  }
  ...
  describe("Random Membership Activity Simulation", function () {
    it("Should simulate random member activities over time", async function () {
      ...
      const { nft, usdt, owner, memberUsers } = await loadFixture(deploySimulationFixture);
      ...
      console.log(`\n✨ การกระทำที่สำเร็จ: ${successfulActions}/${totalActions} (${(successfulActions/totalActions*100).toFixed(1)}%)`);
    ...
    it("Should simulate member upgrade chain scenario", async function () {
      ...
      for (let i = 0; i < chainUsers.length; i++) {
        ...
        console.log(`📝 ${user.address.slice(0, 8)}... ลงทะเบียน (Upline: ${upline.slice(0, 8)}...)`);
      ...
      for (let round = 0; round < 20; round++) {
        ...
        const currentPlan = simulator.memberPlans.get(userToUpgrade.address) || 1;
        
        if (currentPlan < 16) {
          try {
            await simulator.nft.connect(userToUpgrade).upgradePlan(currentPlan + 1);
            simulator.memberPlans.set(userToUpgrade.address, currentPlan + 1);
            
            console.log(`⬆️ ${userToUpgrade.address.slice(0, 8)}... อัพเกรด Plan ${currentPlan} → Plan ${currentPlan + 1}`);
          } catch (error) {
            console.log(`❌ อัพเกรดล้มเหลว: ${error.message}`);
          }
        }
      ...
      for (let i = 0; i < chainUsers.length; i++) {
        ...
        console.log(`   User ${i + 1}: Plan ${plan}, Total Referrals: ${member.totalReferrals}, Total Earnings: ${ethers.formatEther(member.totalEarnings)} USDT`);
      }
    ...
    it("Should simulate cycle completion scenarios", async function () {
      ...
      for (let i = 0; i < batchSize; i++) {
        ...
        console.log(`📝 สมาชิกที่ ${i + 1}: ${user.address.slice(0, 8)}... (Upline: ${upline.slice(0, 8)}...)`);
        ...
        if (newCycleEvent) {
          ...
          console.log(`🔄 รอบใหม่เริ่มขึ้น! Plan ${parsed.args[0]}, Cycle ${parsed.args[1]}`);
        }
        
        // ตรวจสอบสถานะรอบ
        const cycleInfo = await nft.getPlanCycleInfo(1);
        console.log(`   └─ รอบปัจจุบัน: ${cycleInfo[0]}, สมาชิกในรอบ: ${cycleInfo[1]}/4`);
    ...
    it("Should simulate mixed activity with time progression", async function () {
      ...
      for (let day = 1; day <= totalDays; day++) {
        ...
        for (let action = 0; action < actionsPerDay; action++) {
          ...
          await ethers.provider.send("evm_increaseTime", [Math.floor(Math.random() * 3600) + 1800]); // 30 นาที - 1.5 ชั่วโมง
          await ethers.provider.send("evm_mine");
        }
